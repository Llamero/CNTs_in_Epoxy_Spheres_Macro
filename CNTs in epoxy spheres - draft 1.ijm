...
for (i=0; i<fileList.length; i++) {
	...
	for(a=1; a<nStacks+1; a++) {	
		...
		saveAs("Tiff", outputDirectory + BFTitle + " - invert - aligned - 1 blur");
		close();
		...
		open(outputDirectory + BFTitle + " - invert - aligned - 1 blur.tif");
		imageCalculator("Subtract create stack", BFTitle + " - invert - aligned.tif", BFTitle + " - invert - aligned - 1 blur.tif");

		//Save the result, close the original and blurred stack, and delete files if "yes"
		saveAs("Tiff", outputDirectory + BFTitle + " - invert - aligned - minus 1 blur");
		close("*- invert - aligned.tif");
		close("*- invert - aligned - 1 blur.tif");
		if(keepImages){
			File.delete(outputDirectory + BFTitle + " - invert - aligned.tif");
			File.delete(outputDirectory + BFTitle + " - invert - aligned - 1 blur.tif");
		}
